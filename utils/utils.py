...
def normImg(img, if_toClr=True):
	...
	div = (v_max - v_min)
	if div == 0:
		div = 1e-5      # small number
	rst = ((img.astype(float) - v_min) / div * 255).astype(np.uint8)
	if if_toClr and img.ndim < 3:   # ad dim to color
		rst = np.stack([rst, rst, rst], axis=2)
	return rst


def normImg_clip(img, rg = None, if_toClr=True):
	'''
	clip the image as max and min to show
	:param img:
	:param if_toClr: if convert to color
	:return:
	'''
	if rg is None:
		...
		rg = [v_min, v_max]
	v_min, v_max = rg
	img = img.clip(rg[0], rg[1])        # clip to range
...
def get_ptc(depth, f, c=None, bb=None):
	'''
...
def get_ptc_mask(depth, f, mask=None, c=None):
	'''
	get the list of the point cloud in flatten order, row -> column order.
	:param depth: 2d array with real depth value.
	:param f:
...
def get_dp_3d(jts, depth):
	'''
	from the 2d jt and depth map, estimate the depth proxy 3d joints
	:param jts:  should be n_jtx3, last dim for confidence
	:param depth: the depth map  corresponding to the jt definition. depth should be filtered.
	:return: the n_jtx4 with additional dim for the 3d depth proxy, 2.5D with x,y(pix) z(mm)
	'''

	if jts.ndim==1: # if get single joint
		jts = np.expand_dims(jts, axis=0)
	shp = jts.shape()
	assert shp[-1] == 3, 'jts last dim suppose to be 3 with confidence'
	shp[-1] =4 # additional dim for conf
	jts_3d_dp = np.zeros(shp)
	for i, jt in enumerate(jts):
		jts_3d_dp[i,:,:2] = jt[:2]  # 2d
		jts_3d_dp[i,:,-1] = jt[2]   # conf

def arr2U8(arr, l, h):
	'''
	from the input array, and lower and upper bound to get U8 images for cv2 visualization
	:param arr:
	:param l:
	:param h:
	:return:  U8 image(cv2)
	'''
	arr_new = (arr.clip(l, h)-l)/(h-l)*255.
	return arr_new.astype(np.uint8)
