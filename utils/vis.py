...
def vis_keypoints(img, kps, kps_lines, kp_thresh=0.4, alpha=1):
	...
	column format
	:param img:
	:param kps: n_jts x 3, flip to 3 x n_jts inside
	:param kps_lines:
	:param kp_thresh:
	:param alpha:
	:return:
...
def stk_IR_D_PM(D, IR, PM, bb=[160, 80, 180, 340], PM_max=100, d_PM=500, opacity_IR=0.5, opacity_PM=0.8, d_bed=2100,
                eye=[1.0, -1.25, 0.7], pth=None):
	'''
	visualize the IR-D-PM in 3d point cloud format,
	:param D:   depth array  in unit16 (mm)
	...
	fig = go.Figure()
	Z_pm = np.ones(shp) * (-d_bed - d_PM)  # 2264 bed height , higher to cut
	li_z = [Z_pm, Z_pm + 100, Z_pm + 200]
	li_img = [arr_plt, IR_plt, PM_plt]        # one is good, 2 problemaic
	# li_img = [arr_plt, IR_plt]
	for img, z in zip(li_img, li_z[:3]):
	...
	fig.update_layout(
		scene=dict(
			...
			yaxis=dict(range=rg_y, title_text='', showticklabels=False, backgroundcolor="rgb(255, 255, 255)"),
			# zaxis=dict(range=[-d_bed - 600, -d_bed + 1100], title_text='', showticklabels=False,
			zaxis=dict(range=[-d_bed - 600, -d_bed + -300], title_text='', showticklabels=False,
			           backgroundcolor="rgb(255, 255, 255)")),
		width=700,
		scene_camera=cam,
		margin=dict(r=0, l=0, b=0, t=0),  # no margin at all?
...
def genPCK(li_mat, ticks, nms_mth, nms_jt, outFd='output/pcks', svNm='pcks.pdf', layout=[2, 4], if_show=False, figsize=[20, 6], bt=0.2):
	...
	for i in range(n_jt):      # loop all jt/subplots
		...
		for j, mat_t in enumerate(li_mat):        # j mth, i jt
			if i == n_jt - 1:  # if last
				ax.plot(ticks, mat_t[i], label=nms_mth[j], linewidth=3)     # mat_t 6 x11  for method
			else:
	...
	# plt.grid(True)        # only last focus axes will be updated
	if if_show:
		plt.show()
...
def cvShow(img, title='demo'):
	'''
	wrapper func for cv2 show 3 lines. For succint
	:param img:
	:param title:
	:return:
	'''
...
def vis_kps(img, kps, vis_kps, kp_limbs, colors=None, kp_thresh=0.4, alpha=1):
	'''
	along the given kp limbs, plot with given color otherwise, use random color from rainbow map. each limb one color
	:param img: should be cv2 image already
	:param kps: n_jt x2
	:param vis_kps: n_jt
	:param kp_limbs: n_limb x n_seg x 2
	:param kp_thresh:
	:param alpha:
	:return:
	'''
	...
	if colors is None:
		cmap = plt.get_cmap('rainbow')
		colors = [cmap(i) for i in np.linspace(0, 1, n_limb + 2)]
		colors = [(c[2] * 255, c[1] * 255, c[0] * 255) for c in colors]  # to cv2 bgr
	assert len(colors) >= n_limb, 'not enough color provided {}'.format(len(colors))
	# Perform the drawing on a copy of the image, to allow for blending.
	# kp_mask = np.copy(img)      # 256 x 256 x 3
	kp_mask = np.array(img)  # 256 x 256 x 3, type issue  uint8? not good for cv2
	# print('----> kp_mask type', kp_mask.dtype)
	# Draw the keypoints.
	for lb, limb in enumerate(kp_limbs):
		color = colors[lb]  # lb color
		for l in range(len(limb)):
			i1 = limb[l][0]
			i2 = limb[l][1]
			p1 = kps[i1, 0].astype(np.int32), kps[i1, 1].astype(np.int32)
			p2 = kps[i2, 0].astype(np.int32), kps[i2, 1].astype(np.int32)
			if vis_kps[i1] > kp_thresh and vis_kps[i2] > kp_thresh:  # both end visible, dim >1??
				cv2.line(
					kp_mask, p1, p2,
					color=color, thickness=2, lineType=cv2.LINE_AA)
			if vis_kps[i1] > kp_thresh:
				cv2.circle(kp_mask, p1, radius=3, color=color, thickness=-1, lineType=cv2.LINE_AA)
			if vis_kps[i2] > kp_thresh:
				cv2.circle(
					kp_mask, p2,
