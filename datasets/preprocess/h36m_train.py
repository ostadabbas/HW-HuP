...
def h36m_train_extract(dataset_path, openpose_path, out_path, extract_img=False):
    ...
    global_idx = [14, 3, 4, 5, 2, 1, 0, 16, 12, 17, 18, 9, 10, 11, 8, 7, 6]     #  ECCV order from h36m idx ,number is where in global
    # pelvis, lh, lknee, l ankle, r_hip, ...spine neck, jaw, head,  +l shoulder , elbow, wrist, + r.. the jt is mapped to the right hand side.
    # structs we use
    ...
    for user_i in user_list:
        ...
        for seq_i in seq_list:
            ...
            # 3D pose file
            poses_3d = pycdf.CDF(seq_i)['Pose'][0]  # camera distance file
            ...
            for frame_i in range(poses_3d.shape[0]):
                ...
                if frame_i % 5 == 0 and (protocol == 1 or camera == '60457274'):
                    ...
                    scale = 0.9*max(bbox[2]-bbox[0], bbox[3]-bbox[1])/200.  # bb normalized to 200 pixel * 0.9  make bb around 200
                    ...
                    parts_.append(part)
                    Ss_.append(S24)     # 24 joint format, pose 3D seems map to smpl 24
