...
def getImg_dsPM(dsFd=r'G:\My Drive\ACLab Shared GDrive\datasetPM\danaLab', idx_subj=1, modality='IR', cov='uncover',
	...
	:return:
	'''
	npy_nmSet = {'depthRaw', 'PMarray', 'depth_dn'}  # mainly use depth raw and PM array
...
class SLP_RD:  # slp reader
	...
	def __init__(self, opts, phase='train', if_0base=True, if_3d_dp=True, if_SPIN_db=True, rt_hn=0.8):
		'''
		for simplab,
		:param opts:
			phase [train|test]
		...
		if 'simLab' in dsFd:
			...
			pth_3d_dp = 'data/SLP_simLab_3d_dp_h36_hn{}.json'.format(rt_hn)        # 13th for head center
			pth_db_SPIN = 'data/SLP_simLab_SPIN_db_hn{}.npz'.format(rt_hn)
		else:
			n_subj = 102
			d_bed = 2.101
			n_split = 90
			pth_3d_dp = 'data/SLP_danaLab_3d_dp_h36_hn{}.json'.format(rt_hn)
			pth_db_SPIN = 'data/SLP_danaLab_SPIN_db_hn{}.npz'.format(rt_hn)
		...
		self.means = {
			# 'RGB': [0.3875689, 0.39156103, 0.37614644],     # original
			'RGB': [0.485, 0.456, 0.406],       # use std RGB for most cases
			'depth': [0.7302197],
			'depthRaw': [2190.869],
			'IR': [0.1924838],
			'PM': [0.009072126],
		...
		if 'train' == phase:        # default train  / or test
			...
			idx_SPIN_db = range(n_split*45)   #
		elif 'test' == phase:
			idxs_subj = range(n_split, n_subj)
			idx_SPIN_db = range(n_split * 45, n_subj*45)
		elif 'all' == phase:
			idxs_subj = range(n_subj)   # all subj
			idx_SPIN_db = range(n_subj * 45)

		self.idxs_subj = idxs_subj
		idxs_subj_all = range(n_subj)
		if 'simLab' in dsFd:        # for simLab all splits the same 7 subj
			...
			idx_SPIN_db = range(n_subj * 45)  # all selected
			self.sizes['RGB'] = [896, 1600]     # simLab larger
		...
		self.li_joints_gt_RGB = []  # for list generation, keep all select from index
		self.li_joints_gt_IR = []
		## PM, depth homo estimated
		self.li_joints_gt_depth = [] # estimated one, in mm
		...
		if if_3d_dp:
			with open(pth_3d_dp, 'r') as f:
				self.li_joints_3d_dp = np.array(json.load(f))       # n_subj x n_frm x 15 x 4  only uncover, in mm
		# j3d_dp  13 original  +  head(h36) + pelvis
		...
		for i in tqdm(idxs_subj_all, desc='initializing SLP'):
			...
			joints_gt_RGB_t = joints_gt_RGB_t.transpose([2, 1, 0])  # Nx n_jt x 3  ( blocked or not last)
			joints_gt_IR_t = sio.loadmat(os.path.join(dsFd, '{:05d}'.format(i + 1), 'joints_gt_IR.mat'))[
				'joints_gt'].transpose([2, 1, 0])
			if if_0base:
				joints_gt_RGB_t[:,:,:2] = joints_gt_RGB_t[:,:,:2] - 1  # to 0 based
				joints_gt_IR_t[:,:,:2] = joints_gt_IR_t[:, :, :2] - 1   # last for not visible
			...
			joints_gt_depth_t = np.concatenate([joints_gt_depth_t, joints_gt_RGB_t[:, :, 2, None]], axis=2) # last single 2, dim decreased, so add None
		...
		for i in idxs_subj:  # cove last
			for j in range(n_frm):
				for cov in opts.cov_li:  # add pth Descriptor， cov last
		...
		# add the db_SPIN,  with repeat axis=0
		if if_SPIN_db:
			self.db_SPIN = dict(np.load(pth_db_SPIN))   # open after initialziation
			nms = ['imgname', 'center', 'scale', 'part', 'S']
			for nm in nms:  # select the partition ds,  repeat if multi cover
				# print('the db type {}, is {}'.format(nm, type(self.db_SPIN[nm])))
				self.db_SPIN[nm] = self.db_SPIN[nm][idx_SPIN_db]    # select, then repeat
				self.db_SPIN[nm] = self.db_SPIN[nm].repeat(len(opts.cov_li), axis=0)    #  repeat cover case,all follow RGB aligned uncover so image name only uncver
	...
	def get_array_joints(self, idx_smpl=0, mod='depthRaw', if_sq_bb=True):
		...
		jt = joints_gt[id_subj-1][id_frm-1].copy()      # give copy of it
		if if_sq_bb:    # give the s
			bb = getattr(self, 'li_bb_sq_{}'.format(mod))[id_subj - 1][id_frm - 1].copy()   # give copy for safety
		else:
			bb = getattr(self, 'li_bb_{}'.format(mod))[id_subj-1][id_frm-1].copy()
		return arr, jt, bb

	def get_j3d_dp(self, idx):
		'''
		get the j3d depth proxy from depth space
		:param idx:
		:return:
		'''
		id_subj, cov, id_frm = self.pthDesc_li[idx]
		jt = self.li_joints_3d_dp[id_subj-1][id_frm-1].copy()
		return jt

	def get_img_pth(self, idx, mod='RGB'):
		'''
		get the image path from idx, relative to dsFd.
		:param idx:
		:param mod:
		:return:
		'''
		idx_subj, cov, idx_frm = self.pthDesc_li[idx]
		npy_nmSet = {'depthRaw', 'PMarray', 'depth_dn'}  # mainly use depth raw and PM array
		if mod in npy_nmSet:  # npy format
			nmFmt = '{:06}.npy'
		else:
			nmFmt = 'image_{:06d}.png'
		# imgPth = os.path.join(self.dsFd, '{:05d}'.format(idx_subj), mod, cov, nmFmt.format(idx_frm))
		imgPth = os.path.join('{:05d}'.format(idx_subj), mod, cov, nmFmt.format(idx_frm))
		# relative to the SLP
		return imgPth
	...
	def get_array_A2B(self, idx=0, modA='IR', modB='depthRaw', if_uncover=False):
		'''
		...
		'''
		id_subj, cov, id_frm = self.pthDesc_li[idx]  # id for file , base 1
		if if_uncover:  # always gives the uncover, for supervision signal usage
			cov = 'uncover'
