...
if __name__ == '__main__':
    ...
    # idx_care = [38, 42, 43] # ht, jaw , h
    # idx_care = [27, 33, 37] #  rh, rs, thorax
    # idx_care = [27, 33, 37] #  rh, rs, thorax
    idx_care = [9, 33, 37] #  rh_op, rs, thorax
    print('get joints 2d, op hip {}'.format(idx_care), pred_keypoints_2d_t[idx_care])
    img_shape, _, _ = renderer(pred_vertices, camera_translation, img)        # will be 0 ~ 1       with mask and depth
    # add the 38 , 42 and 43  jaw head with b, g,  r
    # kp_h36_hd = pred_keypoints_2d_t[43]
    # print('h36 head coord', kp_h36_hd)
    ...
    colors_rd = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
    for i, clr in zip(idx_care, colors_rd):
        x = int(pred_keypoints_2d_t[i, 0])
        y = int(pred_keypoints_2d_t[i, 1])
        cv2.circle(img_shape, (x, y), 3, clr, thickness=1)
