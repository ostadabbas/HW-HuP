...
SPIN original evaluationi script
Example usage:
```
...
parser.add_argument('--dataset', default='h36m-p1', choices=['h36m-p1', 'h36m-p2', 'lsp', '3dpw', 'mpi-inf-3dhp','SLP'], help='Choose evaluation dataset')
...
# added
parser.add_argument('--li_mod_SLP', default=None, help='If set, save detections to a .npz file')
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    renderer = PartRenderer()       # natural 3D part render
    ...
    joint_mapper_h36m = constants.H36M_TO_J17 if dataset_name == 'mpi-inf-3dhp' else constants.H36M_TO_J14 # from h36 17 ->  lsp +  pelvis, spine, neck(upper)  lsp14+3
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        with torch.no_grad():
            pred_rotmat, pred_betas, pred_camera = model(images)  # pose, shape, camera(ortho projection)
        ...
        if step % log_freq == log_freq - 1:
            if eval_pose:
                ...
                print('Reconstruction Error: ' + str(1000 * recon_err[:step * batch_size].mean()))      # aligned error
                print()
            if eval_masks:
